...
import { qualityPrecheckInputShape, runQualityPrecheck } from './tools/quality-precheck/index.js'
...
// Второй шаг после read_changes — единственный способ прогнать линт/prettier/typecheck
// целевого репозитория перед созданием Jira-задачи.
server.registerTool(
  'quality_precheck',
  {
    description:
      'Second step after read_changes. Requires the sessionId returned by start_session. ' +
      'Detects applicable lint/prettier/typescript tooling in the target repo purely from its ' +
      "config files and package.json scripts, runs each tool's own autofix followed by a recheck, " +
      'and reports a per-category pass/fail status. If none of the three categories are detected, ' +
      'the step is skipped. If unresolved findings remain after autofix, the session is blocked and ' +
      "the response names the project's own fix-errors agent/skill to invoke before retrying.",
    inputSchema: qualityPrecheckInputShape,
  },
  async (input) => runQualityPrecheck(input),
)
