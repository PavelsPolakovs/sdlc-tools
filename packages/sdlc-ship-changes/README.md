...
Оставшийся инструмент пайплайна, к реализации в следующих срезах:

- `transition_issue` — перевод Jira-задачи по статусам workflow

Промежуточные шаги полного пайплайна (`create_jira_task`, `create_branch`, `commit`, `open_mr`, `poll_ci`) пока не реализованы и намеренно убраны из `StepName`/`PIPELINE_ORDER` (см. `CLAUDE.md`) — держать их в типах до реальной реализации только мешало бы. `quality_precheck` временно ведёт напрямую к `report`; вернуть их в этот список по мере реальной реализации.
