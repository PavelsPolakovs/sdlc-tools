...
Proof of concept. Сейчас реализовано три инструмента: `start_session` (точка входа пайплайна), `read_changes` (структурирование диффа рабочего дерева) и `ship_report` (финальный шаг) — это доказывает путь install → build → register → call целиком. Остальные инструменты пайплайна (см. Roadmap) — заглушки, ещё не реализованы.
